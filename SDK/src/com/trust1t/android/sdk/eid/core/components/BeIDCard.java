 ...
 * interaction, such as obtaining VERIFY_PIN and PUK codes for authentication, signing,
 * verifying, changing VERIFY_PIN codes, and for notifying the user of the progress of
 ...
 * available, for example, when needing to acquire VERIFY_PIN codes from the user, and
 * the card is in a CCID-compliant Secure Pinpad Reader the VERIFY_PIN entry features
 ...
 * because of user VERIFY_PIN interaction), register an instance of BeIDCardListener
...
public class BeIDCard {
    ...
    public byte[] sign(final byte[] digestValue, final BeIDDigest digestAlgo,
                       final FileType fileType, char[] pin)
            throws CardException, IOException, InterruptedException
             {
                 Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Entering sign method");
        ...
        try {
            ...
            Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Selected algorithm and key");
            if (FileType.NonRepudiationCertificate.getKeyId() == fileType
                    .getKeyId()) {
                Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Verifying pin " + pin + " in sign method");
                PinResult result = verifyPin(PINPurpose.NonRepudiationSignature, pin);
                Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Pin result is " + result.isSuccess());
                if(!result.isSuccess()){
            ...
            Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Signing hash " + digestValue);
            responseApdu = transmitCommand(
                    BeIDCommandAPDU.COMPUTE_DIGITAL_SIGNATURE, digestInfo
                    .toByteArray());
            Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Response from sign is sw " + responseApdu.getSW());
            if (0x9000 == responseApdu.getSW()) {
            ...
            if(sw > 0x6100 && sw < 0x6200) {
                ...
                Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Sending same command but with length " + length);
            ...
            Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "We do need the pin code verification");
            PinResult result = verifyPin(PINPurpose.AuthenticationSignature, pin);
            Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Signing hash " + digestValue);
            responseApdu = transmitCommand(
                    BeIDCommandAPDU.COMPUTE_DIGITAL_SIGNATURE, digestInfo
                    .toByteArray());
            ...
            Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Response from second sign is sw " + responseApdu.getSW());
            if(sw > 0x6100 && sw < 0x6200) {
                int length = sw - 0x6100;
                Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Sending special command with length " + length);
                responseApdu = transmit(new CommandAPDU(0x00, 0xC0, 0x00, 0x00,
                        length));
                if(responseApdu.getSW() == 0x9000){
                    return responseApdu.getData();
	...
	/**public byte[] sign(final byte[] digestValue, final BeIDDigest digestAlgo,
			final be.fedict.commons.eid.client.FileType fileType, final boolean requireSecureReader)
			throws CardException, IOException, InterruptedException {
		if (!fileType.isCertificateUserCanSignWith()) {
			throw new IllegalArgumentException(
					"Not a certificate that can be used for signing: "
		...
		try {
			...
			if (be.fedict.commons.eid.client.FileType.NonRepudiationCertificate.getKeyId() == fileType
					.getKeyId()) {
				this.logger
			...
			if (0x9000 == responseApdu.getSW()) {
				/*
				 * OK, we could use the card VERIFY_PIN caching feature.
				 *
				 * Notice that the card VERIFY_PIN caching also works when first doing
			...
			/*
			 * 0x6982 = Security constants not satisfied, so we do a VERIFY_PIN
			...
			this.logger
					.debug("computing digital signature (attempt #2 after VERIFY_PIN verification)...");
			responseApdu = transmitCommand(
					BeIDCommandAPDU.COMPUTE_DIGITAL_SIGNATURE, digestInfo
							.toByteArray());
			if (0x9000 != responseApdu.getSW()) {
	 ...
	 *            citizen's VERIFY_PIN if false, the current BeIDCardUI will be used in
...
/**	public byte[] signAuthn(final byte[] toBeSigned,
		...
		final byte[] digest = messageDigest.digest(toBeSigned);
		return this.sign(digest, BeIDDigest.SHA_1,
				be.fedict.commons.eid.client.FileType.AuthentificationCertificate, requireSecureReader);
	}*/

	/**
	 * Verifying VERIFY_PIN Code (without other actions, for testing VERIFY_PIN), using the
	 * most secure method available. Note that this still has the side effect of
	 * loading a successfully tests VERIFY_PIN into the VERIFY_PIN cache, so that unless the
	 * card is removed, a subsequent authentication attempt will not request the
	 * VERIFY_PIN, but proceed with the VERIFY_PIN given here.
	 *
	 * @throws java.io.IOException
	 * @throws com.precisebiometrics.android.mtk.api.smartcardio.CardException
	 * @throws InterruptedException
	...
	/**
	 * Discard the citizen's VERIFY_PIN code from the VERIFY_PIN cache. Any subsequent
	 * Authentication signatures will require VERIFY_PIN entry. (non-repudation
          ...
          final ResponseAPDU responseApdu = transmitCommand(
           ...
           final int sw = responseApdu.getSW();
            ...
            if(sw > 0x6000 && sw < 0x7000) {
                ...
                length = sw - 0x6C00;
                if(length == 0xF4 || length == 0xB4 || length == 0x74 || length == 0x34)
                {
    ...
    private ByteArrayOutputStream tactivoFailureTempSolution(int length, int offset, ByteArrayOutputStream baos){
        byte[] data = null;
    ...
    public PinResult verifyPin(final PINPurpose purpose, char[] pin) throws IOException,
            CardException, InterruptedException {
        PinResult output = new PinResult();
        Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Received request to verify pin " + new String(pin));
        ResponseAPDU responseApdu;
        int retriesLeft = -1;
        Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Calling verifypin method");
        responseApdu = verifyPIN(retriesLeft, purpose, pin);
        Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Received response from card with sw " + responseApdu.getSW());
        if (0x9000 != responseApdu.getSW()) {
    ...
    private ResponseAPDU verifyPIN(final int retriesLeft,
        ...
        Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Going to send apdu");
