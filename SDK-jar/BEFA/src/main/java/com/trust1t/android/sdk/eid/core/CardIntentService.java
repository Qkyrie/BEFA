...
public class CardIntentService extends NonStopIntentService {
	...
	public void setBeIDCard(BeIDCard beIDcard) {
		...
		Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Beid card set is "
				+ beIDcard.getATR() + " " + beIDCard.getATR());
	}
		...
		try {
			if (beIDCard != null) {
				Log.d(Configuration.CARD_SERVICE_LOG_FILTER,
						"Intent operation passed is "
								+ intent.getStringExtra(CardIntent.CARD_OPERATION));
				if (intent.getStringExtra(CardIntent.CARD_OPERATION).equals(
						CardIntent.READ_FILE)) {
					FileType type = FileType.valueOf(intent
							.getStringExtra(CardIntent.FILE_TYPE));
					Log.d(Configuration.CARD_SERVICE_LOG_FILTER,
							"Reading file type " + type.name());
					byte[] data = null;
					try {
						data = beIDCard.readFile(type);
					} catch (Exception e) {
						Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Error trying to read BEIDCard");
					}
					sendFileData(data, type);
				} else if (intent.getStringExtra(CardIntent.CARD_OPERATION)
						.equals(CardIntent.VERIFY_PIN)) {
					String pin = intent.getStringExtra(CardIntent.PIN);
					Log.d(Configuration.CARD_SERVICE_LOG_FILTER,
							"Verifying pin");
					PinResult result = beIDCard.verifyPin(PINPurpose.PINTest,
							pin.toCharArray());
					sendPinResult(result);
				} else if (intent.getStringExtra(CardIntent.CARD_OPERATION)
						.equals(CardIntent.SIGN)) {
					String hash = intent
							.getStringExtra(CardIntent.HASH_TO_SIGN);
					String pin = intent.getStringExtra(CardIntent.PIN);
					Log.d(Configuration.CARD_SERVICE_LOG_FILTER,
							"Received request to sign hash " + hash);
					byte[] result = beIDCard.sign(hash.getBytes(),
							BeIDDigest.SHA_512,
					...
					byte[] hashToSign = intent
							.getByteArrayExtra(CardIntent.HASH_TO_SIGN);

					Log.i("CardIntentService",
							"the hash (encoded for showing) to sign is "
									+ Base64.encodeToString(hashToSign,
											Base64.DEFAULT));
			...
			if (e.getMessage().contains("SCARD_E_NO_SMARTCARD")) {
				Log.e(Configuration.CARD_SERVICE_LOG_FILTER,
						"Card was removed, setting card instance to null");
				beIDCard = null;
			}
			sendException(e.getMessage());
			Log.e(Configuration.CARD_SERVICE_LOG_FILTER,
					"CardException thrown in CardIntentService", e);
		} catch (IOException e) {
			sendException(e.getMessage());
			Log.e(Configuration.CARD_SERVICE_LOG_FILTER,
					"IOException thrown in CardIntentService", e);
		} catch (InterruptedException e) {
			sendException(e.getMessage());
			Log.e(Configuration.CARD_SERVICE_LOG_FILTER,
					"InterruptedException thrown in CardIntentService", e);
		} catch (NoSuchAlgorithmException e) {
			sendException(e.getMessage());
			Log.e(Configuration.CARD_SERVICE_LOG_FILTER,
					"NoSuchAlgorithmException thrown in CardIntentService", e);
		}
	}
	...
	public class LocalBinder extends Binder {
		public CardIntentService getService() {
			Log.d(Configuration.CARD_SERVICE_LOG_FILTER,
					"Cardintentservice's binder is called");
	...
	private void sendPinResult(PinResult result) {
		...
		broadcastIntent.putExtra(CardIntent.CARD_OPERATION_RESPONSE,
				CardIntent.PIN_RESPONSE);
		broadcastIntent.putExtra(CardIntent.PIN_RESULT, result);
		sendBroadcast(broadcastIntent);
	}

	private void sendFileData(byte[] data, FileType type) {
		Log.d(Configuration.CARD_SERVICE_LOG_FILTER, "Returning filedata for  "
				+ type);
		Intent broadcastIntent = new Intent();
		broadcastIntent.setAction(Configuration.CARD_SERVICE_BROADCAST_ACTION);
		broadcastIntent.addCategory(Intent.CATEGORY_DEFAULT);
		broadcastIntent.putExtra(CardIntent.CARD_OPERATION_RESPONSE,
				CardIntent.FILE_READ_RESPONSE);
	...
	private void sendSignedHash(byte[] hash, String sign_type) {
		...
		broadcastIntent.addCategory(Intent.CATEGORY_DEFAULT);
		broadcastIntent.putExtra(CardIntent.CARD_OPERATION_RESPONSE,
				CardIntent.SIGN_RESPONSE);
		if (sign_type.equals(CardIntent.SIGN_AUTH))
			broadcastIntent.putExtra(CardIntent.SIGN_TYPE_RESPONSE,
					CardIntent.SIGN_AUTH);
